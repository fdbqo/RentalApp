...
export default function ListingsScreen() {
  ...
  const PropertyList = useMemo(() => {
    ...
    if (isWeb) {
      return (
        <ScrollView showsVerticalScrollIndicator={false}>
          {isHardcodedProperty && (
            <XStack alignItems="center" marginBottom="$4" backgroundColor="$yellow2" padding="$2" borderRadius="$2">
              <AlertTriangle color="$yellow10" size={20} />
              <Text marginLeft="$2" color="$yellow10">No properties found. Showing a sample property.</Text>
            </XStack>
          )}
    ...
    return (
      <FlatList
        ListHeaderComponent={isHardcodedProperty ? (
          <XStack alignItems="center" marginBottom="$4" backgroundColor="$yellow2" padding="$2" borderRadius="$2">
            <AlertTriangle color="$yellow10" size={20} />
            <Text marginLeft="$2" color="$yellow10">No properties found. Showing a sample property.</Text>
          </XStack>
